...
async function loadquetion(testid) {
    ...
    allquetionsdata.forEach(snap => {
        ...
        newdiv.className = "result"; // Add result class for styling
        newdiv.innerHTML = snap.data().content;
        
        // Enhance style of select elements if present
...
document.getElementById("yesconfirmsubmit").addEventListener("click", async () => {
    ...
    questions.forEach((q, index) => {
        ...
        let questionDiv = document.querySelector("#result").children[index]; // Better targeting
        
        if (userAnswer.value == q.co) {
            questionDiv.style.border = "2px solid rgba(74, 222, 128, 0.5)"; // Green border
            testscore++;
        }
        else {
            questionDiv.style.border = "2px solid rgba(248, 113, 113, 0.5)"; // Red border
            
            // Re-enable this if you want to keep the answer visible
             userAnswer.disabled = true; 
            
            // userAnswer.remove(); // Don't remove for better UX? Original code removed it.
            // Let's stick to original logic but stylize
    ...
    // Optimizing firestore batch if possible, but keeping original logic for now
    ...
    // NOTE: In the new UI, we want to show the submittedform DIV, not the result list again?
    // Original code:
    // document.getElementById("result").style.display = "block"; (This shows the questions with corrections)
    // document.getElementById("submitedform").style.display = "block"; (This shows the score)
    ...
    document.getElementById("confirmsubmit").style.display = "none";
    document.getElementById("submitedform").style.display = "block"; // Show Success Card
